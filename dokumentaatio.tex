...
% $Id: dokumentaatio.tex,v 1.27 2004-09-23 14:02:07 jatoivol Exp $
...
inside the system. It is set automatically by the \cfunction{new_variable} 
...
variables. Both \cdatatype{varlink} and \cdatatype{Variable_iterator} are
...
calling \examplecode{next_Variable(iterator)}. On the other hand, iterators
...
variable iterator~\cparameter{it} for a variable identified by the string
\cparameter{symbol}. The iterator will be used by the function, so it will
...
% miten m‰‰ritell‰‰n optimaalisuus
...
undirected links. The resulting graph is called a {\it moral
graph}.
...
in the \cstructfield{cardinality} array (or one if the number of variables 
equals zero).
...
the \cstructfield{size\_of\_data} field. The data in the array is arranged
according to the order of the corresponding variables: the variable
which has its cardinality first in the \cstructfield{cardinality}
array is considered least significant in the sense of placing a value 
in the \cstructfield{data} array. 
\end{description}
...
% <JJT: n‰in pitk‰lle p‰‰sin s‰‰t‰misess‰>
