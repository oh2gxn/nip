...
% $Id: doc.tex,v 1.10 2011-01-22 22:25:56 jatoivol Exp $
...
\item[nip\_inverse\_mapping(p, flat\_index, indices)] transforms a
  ...
  \cparameter{indices} array (of size at least \cstructfield{num\_of\_vars})
...
in the future development\footnote{although C has poor support for
  polymorphism}. The \cdatatype{nip\_clique} and \cdatatype{nip\_sepset}
...
\item[nip\_new\_clique(vars, num\_of\_vars)] creates a
  ...
  The clique will not have any adjacent sepsets initially: sepsets must be
  created and added separately. Typically, a join tree is created in the
  following order: first, a suitable set of cliques is created, a set of
  candidate sepsets is created, subset of the sepsets are selected and
  added to the cliques, and finally possible useless candidate sepsets are
  freed.
...
\item[nip\_confirm\_sepset(s)] adds the \cdatatype{nip\_sepset}
  \cparameter{s} to the cliques referenced during
  \cfunction{nip\_new\_sepset}. This makes the candidate sepset a final
  part of the join tree and ensures that both the sepset and the neighbour
  cliques reference each other properly. On the other hand, if the sepset
  is not confirmed to be a part of a join tree, the neighbouring cliques
  will not reference it and no message passing algorithm will reach
  \cparameter{s}. This was made in order to have several possible candidate
  sepsets before selecting the best ones to be included, while enforcing
  proper bi-directional references. Make sure not to confirm sepsets that
  would create loops in the join tree.

\item[nip\_new\_sepset(neighbour\_a, neighbour\_b)] creates a
  new \cdatatype{nip\_sepset} between cliques \cparameter{neighbour\_a} and
  \cparameter{neighbour\_b}. This usually means that all cliques
  must be created before sepsets. Note that
  \cfunction{nip\_confirm\_sepset} needs to be called in order to include
  the sepset in the join tree.
  
\item[nip\_free\_sepset(s)] just frees the memory allocated for the
  sepset \cparameter{s}. This is called automatically by 
  \cfunction{nip\_free\_clique} so usually this function is needed only for
  freeing possible useless \cdatatype{sepsets} which are not included in the 
  join tree (via \cfunction{nip\_confirm\_sepset}).

\item[nip\_create\_potential(variables, num\_of\_vars, data)] can help if
  you need to create potentials out of \cdatatype{double arrays} like the
...
\item[nip\_unmark\_clique(c)] resets the mark in the
  clique~\cparameter{c}. Note that this has to be done for every clique
  before calling \cfunction{nip\_distribute\_evidence},
  \cfunction{nip\_collect\_evidence}, or
  \cfunction{nip\_cliques\_connected}.

\item[nip\_clique\_size(c)] tells how many associated variables  
  clique~\cparameter{c} has.

\item[nip\_sepset\_size(s)] tells how many associated variables  
  sepset~\cparameter{s} has.

\item[nip\_cliques\_connected(one, two)] runs a depth-first search in the
  join tree to find out if cliques~\cparameter{one} and \cparameter{two}
  are connected (in the same join tree) or not. Remember to unmark all the
  cliques before calling this.

\item[nip\_distribute\_evidence(c)] implements the algorithm which spreads 
  the evidence entered in to the cliques away from the given 
  clique~\cparameter{c}. See \cite{Huang1994} page 21 for
  details. Remember to unmark all the cliques before calling this.

\item[nip\_collect\_evidence(c1, s12, c2)] implements the algorithm which
  collects evidence from the subtree rooted in the given
  clique~\cparameter{c2} and passes the evidence to 
  clique~\cparameter{c1}. Sepset~\cparameter{s12} is the sepset between the two
...
\item[nip\_gather\_joint\_probability(start, vars, n\_vars, isect,
  n\_isect)] is a recursive function for computing the joint probability
  distribution of the \cparameter{n\_vars} variables contained by the
  \cdatatype{variable array} \cparameter{vars}. The recursion begins (or
  continues) from the \cparameter{start} clique.
  ...
  \examplecode{nip\_gather\_joint\_probability(c, vars, n, NULL, 0);}\\ 
  where \examplecode{c} is some clique and \examplecode{vars} contains the
  \examplecode{n} variables of interest. The function returns a potential
  which is ordered accoding to the given variables, so the result depends
  on the order of \examplecode{vars} array.
...
% Vois olla, että heapin buildaus olis n log n?
% Näyttäisi siltä, että trianguloinnissa tehdään n kertaa looppi:
% Tässä loopissa tehdään yksi kappale extract_miniä (joka on k log n?)
% höpsis, clean_item vie täyden k rundia, eli nnk
% no onpa vaikea.
...
\VerbatimInput{../examples/model.net}
