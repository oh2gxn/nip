...
% $Id: doc.tex,v 1.5 2010-11-26 17:06:01 jatoivol Exp $
...
{\it join tree}. Various operations for handling join trees are located in
{\it clique.h} and {\it clique.c}. Abstractions provided by potential,
clique, graph and variable subsystems are used by the {\it parser}
subsystem for reading models and data from input files.  Finally the {\it
  nip} subsystem (files nip.h and nip.c) provides abstraction for time
...
between random variables. This is achieved by {\it graphs}, {\it
  potentials}, and {\it cliques} presented in the following
...
\item[symbol] is a short string used for identifying the variable. Maximum
  length is defined as \cdefine{NIP\_VAR\_TEXT\_LENGTH} in
  \cfilename{nipvariable.h}. To ensure the correct operation of the system,
...
\item[id] is an \cdatatype{unsigned long} used for identification
  internally by the system. It is set automatically by the
  \cfunction{new\_variable} function and should not be tampered after that.
...
\item[parents] is an array of references to the parent variables
  (\examplecode{nip\_variable[]}) or a null pointer if there are no parents
  for this variable.
...
\item[nip\_new\_variable(symbol, name, states, cardinality)] creates a
  ...
  strings may be truncated to the allowed length. The states of the new
  variable will be named according to the given string array
...
% replaced by NIP\_CARDINALITY() macro
...
\item[nip\_new\_interface\_list()] creates a new empty list of
  ...
  variables, while some of them are yet to be created.
...
\item[nip\_variable\_list\_to\_array(l)] takes the linked list of variables
  \cparameter{l} and puts them into a newly allocated array in the same
  order. Size of the array can be found with
...
of distributions can be considered as functions from the N-dimensional space
...
\item[nip\_random\_potential(p)] makes the potential \cparameter{p} random
  by assigning a pseudorandom value ($rand \in [0,1]$) to each element of
  the table.
...
\item[nip\_set\_potential\_value(p, indices, value)] sets a
  \cdatatype{double} \cparameter{value} in the
  \cdatatype{potential}~\cparameter{p} the same way as
  \cfunction{nip\_get\_potential\_value(p, indices)} retrieves it.  (See
  the description above.)
  
\item[nip\_inverse\_mapping(p, flat\_index, indices)] transforms a
  so-called flat index into the \cdatatype{integer array}
  \cparameter{indices} according to the
  \cdatatype{potential}~\cdatatype{p}.  A flat index is an index used for
  addressing data in the potential and is therefore an integer between
  \examplecode{0} and \examplecode{p->size\_of\_data - 1}
  inclusive. Shortly, this function is made for finding out which
  configuration of states of the variables corresponds to each element in
  the potential. Note that the \cparameter{indices} array is not allocated
  by the function.
...
\item[nip\_total\_marginalise(source, destination, variable)] can be used
  ...
  one of the variables is left unmarginalised. The parameter
...
\item[nip\_normalise\_cpd(p)] normalises the potential \cparameter{p} so
  ...
  assumes that the first variable (dimension) is the child variable. The
  assumption makes it simpler and faster than 
...
and use was therefore less intuitive. Note that our join tree
